  ...
  const hardware_interface::HardwareInfo & info)
{
  ...
  // Create logger
  auto logger = rclcpp::get_logger("DiffBotSystemHardware");
  ...
  for (const hardware_interface::ComponentInfo & joint : info_.joints)
  {
    ...
    {
      // Replace all get_logger() with logger
      RCLCPP_FATAL(logger, "Invalid joint interface configuration for '%s'", joint.name.c_str());
      return hardware_interface::CallbackReturn::ERROR;
    }
  }
...
{
  auto logger = rclcpp::get_logger("DiffBotSystemHardware");
  RCLCPP_INFO(logger, "Activating hardware, connecting to ESP32...");
